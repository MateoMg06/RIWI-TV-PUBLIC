...
class AuthService implements IAuthService {
  ...
  async register(dto: RegisterDto): Promise<{ message: string; userId: number }> {
    ...
    try {
      ...
      // 3. Crear membresía inicial (por defecto nivel bronce con 0 puntos)
      ...
      // 4. Crear el QR de la membresía (único e intransferible, amarrado a la membresía)
  ...
  // genera un token unico para el QR, revisa que no exista en la bd
  private async generateUniqueQrToken(): Promise<string> {
    const chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';
    // intento 5 veces por si el token se repite
    ...
    // si se repitio todo, agrego la fecha para hacerlo unico
